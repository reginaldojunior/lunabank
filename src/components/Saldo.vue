...
export default {
    ...
    methods: {
        async enviarTransferencia() {
            ...
            await axios.post('https://pix-api-simulator-cc50fa0d09e3.herokuapp.com/transfere/origem/NOT_FOUND/destino/TESTE/valor/1000', { headers })
            await axios.get('https://pix-api-simulator-cc50fa0d09e3.herokuapp.com/chaves_pix', { headers })
        },
        async chavesPix() {
            const token = VueCookies.get('token')
            const headers = { Authorization: `Bearer ${token}` }
        ...
        async carregarDados() {
            try {
                ...
                // Buscar saldo
                const saldoRes = await axios.get('https://pix-api-simulator-cc50fa0d09e3.herokuapp.com/saldo', { headers })
                this.saldoAtual = saldoRes.data.saldo / 100
                this.chave = saldoRes.data.usuario.chave
                this.usuarioId = saldoRes.data.usuario.id

                // Buscar movimentações
                const movRes = await axios.get('https://pix-api-simulator-cc50fa0d09e3.herokuapp.com/extrato', { headers })
        ...
        async fazerPix() {
            try {
                ...
                await axios.post('https://pix-api-simulator-cc50fa0d09e3.herokuapp.com/transfere/destino/' + this.pixDestino + '/valor/' + Math.round(this.pixValor * 100), {}, { headers })
