...
export default {
    ...
    methods: {
        async enviarTransferencia() {
            ...
            await axios.post('https://12cc9d275efc.ngrok-free.app/transfere/origem/NOT_FOUND/destino/TESTE/valor/1000', { headers })
            await axios.get('https://12cc9d275efc.ngrok-free.app/chaves_pix', { headers })
        },
        async chavesPix() {
            const token = VueCookies.get('token')
            const headers = { Authorization: `Bearer ${token}` }
        ...
        async carregarDados() {
            try {
                ...
                // Buscar saldo
                const saldoRes = await axios.get('https://12cc9d275efc.ngrok-free.app/saldo', { headers })
                this.saldoAtual = saldoRes.data.saldo / 100
                this.chave = saldoRes.data.usuario.chave
                this.usuarioId = saldoRes.data.usuario.id

                // Buscar movimentações
                const movRes = await axios.get('https://12cc9d275efc.ngrok-free.app/extrato', { headers })
        ...
        async fazerPix() {
            try {
                ...
                await axios.post('https://12cc9d275efc.ngrok-free.app/transfere/destino/' + this.pixDestino + '/valor/' + Math.round(this.pixValor * 100), {}, { headers })
